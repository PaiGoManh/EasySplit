...
const UpdateGroup = () => {
  ...
  const fetchGroupDetails = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/group/getgroup/${groupId}`, {
        method: 'GET',
        credentials: 'include',
      });
      ...
      setGroupName(data.group.name);  // Access the group object in the response data
      setGroupDescription(data.group.description);  // Access the group object in the response data
      setCurrency(data.group.currency);  // Access the group object in the response data
  ...
  const handleUpdateGroup = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`/api/group/updategroup/${groupId}`, {
        method: 'PUT',
        headers: {
      ...
      <form onSubmit={handleUpdateGroup} className="flex flex-wrap -mx-2">
                ...
                {selectedMembersList.map((memberId) => {
                  ...
                  return (
                    <li key={memberId} className="flex justify-between items-center mb-2">
                      {member?.username}  {/* Ensure member is not null or undefined */}
                      <button
                        type="button"
        ...
        <div className="w-full px-2">
          <div className="flex justify-between">
            ...
            <button 
              type="button" 
              onClick={() => navigate('/Dashboard/groups')}  // Add navigation on cancel
